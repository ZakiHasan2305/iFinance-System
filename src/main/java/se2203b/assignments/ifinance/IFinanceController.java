...
public class IFinanceController implements Initializable {
    ...
    private AccountGroupsAdapter accountGroups;
    private AccountCategoryAdapter accountCategory;

    @FXML
    void openGroups() throws SQLException, IOException {
        accountGroups = new AccountGroupsAdapter(conn, false);
        accountCategory = new AccountCategoryAdapter(conn,false);
        FXMLLoader fxmlLoader = new FXMLLoader(IFinanceController.class.getResource("AccountGroups-view.fxml"));
        Parent accGroups = fxmlLoader.load();
        ...
        accountGroupsController.setUserModel(accountGroups,accountCategory);


        // create new stage
        Stage stage = new Stage();
        // add the about's UI elements to the stage
        stage.setScene(new Scene(accGroups));
